...
class HygrothermographCgdk2Accessory {
  constructor(log, config) {
    ...
    this.temperatureAlarmService = this.getTemperatureAlarmService();
  ...
  setTemperatureStatus(temperature) {
    this.log.warn("temperature Status.");
    if (temperature == null) {
      this.temperatureStatus = undefined;
    } else if (temperature > this.temperatureThreshold) {
      this.tempStatus = Characteristic.StatusTampered.TAMPERED;
      this.alarm();
    } else {
      this.tempStatus = Characteristic.StatusTampered.NOT_TAMPERED;
      this.resetNotification()
    }
    this.log.warn("temperature Status: " + temperature);
  }
  ...
  get temperatureThreshold() {
    return this.config.hightTemperature || 6;
  }

  get isAlarmTemperatureEnabled() {
    return this.config.alarmTemperatureEnabled || false;
  }
  ...
  setupScanner() {
    ...
    scanner.on("temperatureChange", (temperature, peripheral) => {
      ...
      this.setTemperatureStatus(temperature);
    });
  ...
  getTemperatureAlarmService() {
    const temperatureAlarmService = new Service.TemperatureSensor(
      this.temperatureName
    );
    temperatureAlarmService
      .getCharacteristic(Characteristic.CurrentTemperature)
      .on("get", this.onCharacteristicGetValue.bind(this, "temperatureStatus"));

    return temperatureAlarmService;
  }
  ...
  sendNotification() {
    // TODO 
    this.log.error("send alert notification");
